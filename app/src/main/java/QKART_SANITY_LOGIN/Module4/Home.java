...
public class Home {
    ...
    public Boolean changeProductQuantityinCart(String productName, int quantity) {
        try {
            ...
            List<WebElement> cartPructList =
                    driver.findElements(By.xpath("//div[@class='MuiBox-root css-1gjj37g']"));
            for (WebElement productDetails : cartPructList) {
                WebElement productTitle = productDetails.findElement(By.xpath("./div[1]"));
                String actualProductName = productTitle.getText();
                if (actualProductName.equals(productName)) {
                    while (true) {
                        WebElement actualQuantityElement =
                                productDetails.findElement(By.xpath("./div[2]/div[1]/div"));
                        String actualQuantityText = actualQuantityElement.getText();
                        int actualQantity = Integer.parseInt(actualQuantityText);
                        if (quantity > actualQantity) {
                            WebElement plusButton = productDetails
                                    .findElement(By.xpath("./div[2]/div[1]/button[2]"));
                            plusButton.click();
                            Thread.sleep(5000);
                        } else if (quantity < actualQantity) {
                            WebElement minusButton = productDetails
    ...
    public Boolean verifyCartContents(List<String> expectedCartContents) {
        try {
            ...
            WebElement cartParent =
                    driver.findElement(By.xpath("//*[@id=\"root\"]/div/div/div[3]/div[2]/div"));
